...
1. [Creating Data Model](#section-2)
...
This read me is used to document the progress ...
...
The first step to the project is data loading and preparation.

The dataset set consists of 4 tables: an Orders fact table and three dimensions tables for Products, Customers and Stores. Each of these are stored in different ways and so require different ways to import them into Power BI.
...
- Split Shipping date into date and time columns
...
The Products table is stored as a csv file.
...
The Stores table is stored in Azure Blob Storage.

When initially imported into Power BI, it appears as a single row table. 
The table is accessed by entering the Power Query Editor view and clicking on the link under content to gain access to the csv file contained in the storage.
...
The Customer table is stored as three seperate files in a folder.
...
So I changed the date table to end of the second quarter in 2023
...
Format the card visuals so that they are the same size as the gauges, and the text is centered.
...
Note: ive also turned off interaction of the the slicer to the customer line chart
...
- For each button, go to Format > Button Style > Apply settings to and set it to On Hover, and then select the alternative colourway of the relevant button under the Icon tab
    - I also went back to produt page and added this to the filter icon as i initially hadnt done this
